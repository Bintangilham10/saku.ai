CREATE TABLE public.ai_rate_limits (
  user_id uuid PRIMARY KEY REFERENCES public.app_users(id) ON DELETE CASCADE,
  window_started_at timestamptz NOT NULL,
  request_count integer NOT NULL CHECK (request_count > 0),
  updated_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.ai_rate_limits ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON public.ai_rate_limits FROM authenticated;

CREATE OR REPLACE FUNCTION public.consume_ai_rate_limit(
  request_limit integer DEFAULT 10,
  window_seconds integer DEFAULT 60
)
RETURNS TABLE(allowed boolean, retry_after_seconds integer)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public
AS $$
DECLARE
  app_user_id uuid;
  current_time timestamptz := clock_timestamp();
  rate_limit public.ai_rate_limits%ROWTYPE;
BEGIN
  IF request_limit <= 0 OR window_seconds <= 0 THEN
    RAISE EXCEPTION 'Rate limit configuration must be positive';
  END IF;

  app_user_id := public.requesting_app_user_id();

  IF app_user_id IS NULL THEN
    RAISE EXCEPTION 'Unauthorized' USING ERRCODE = '42501';
  END IF;

  INSERT INTO public.ai_rate_limits (
    user_id,
    window_started_at,
    request_count,
    updated_at
  ) VALUES (
    app_user_id,
    current_time,
    1,
    current_time
  )
  ON CONFLICT (user_id) DO UPDATE
  SET
    window_started_at = CASE
      WHEN current_time >= ai_rate_limits.window_started_at
        + make_interval(secs => window_seconds)
        THEN current_time
      ELSE ai_rate_limits.window_started_at
    END,
    request_count = CASE
      WHEN current_time >= ai_rate_limits.window_started_at
        + make_interval(secs => window_seconds)
        THEN 1
      ELSE LEAST(ai_rate_limits.request_count + 1, request_limit + 1)
    END,
    updated_at = current_time
  RETURNING * INTO rate_limit;

  allowed := rate_limit.request_count <= request_limit;
  retry_after_seconds := CASE
    WHEN allowed THEN 0
    ELSE GREATEST(
      1,
      CEIL(
        EXTRACT(
          EPOCH FROM (
            rate_limit.window_started_at
              + make_interval(secs => window_seconds)
              - current_time
          )
        )
      )::integer
    )
  END;

  RETURN NEXT;
END;
$$;

CREATE OR REPLACE FUNCTION public.set_ai_consent(consent boolean)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public
AS $$
DECLARE
  app_user_id uuid;
BEGIN
  app_user_id := public.requesting_app_user_id();

  IF app_user_id IS NULL THEN
    RAISE EXCEPTION 'Unauthorized' USING ERRCODE = '42501';
  END IF;

  UPDATE public.app_users
  SET settings = jsonb_set(settings, '{aiConsent}', to_jsonb(consent), true)
  WHERE id = app_user_id;

  RETURN FOUND;
END;
$$;

REVOKE ALL ON FUNCTION public.consume_ai_rate_limit(integer, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.set_ai_consent(boolean) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.consume_ai_rate_limit(integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_ai_consent(boolean) TO authenticated;
